...
def _apply_pooling_block(params, source: int, target: int) -> None:
    """
    Apply a trainable QCNN pooling block.

    The source wire is coupled into the target wire and then removed from the
    active wire set by the caller. This follows the common QCNN pattern of
    reducing the number of active qubits between convolutional stages while
    keeping the full simulation unitary.
    """
    qml.CNOT(wires=[source, target])
    qml.CRY(params[0], wires=[source, target])
    qml.CRZ(params[1], wires=[source, target])
    qml.CNOT(wires=[target, source])
...
def _apply_qcnn(params) -> None:
    ...
    _apply_pooling_block(params["pool1"][0], 0, 1)
    _apply_pooling_block(params["pool1"][1], 2, 3)
    _apply_convolution_block(params["conv2"][0], 1, 3)
    _apply_pooling_block(params["pool2"][0], 1, 3)
...
def _flatten_params(params: dict[str, np.ndarray]) -> tuple[pnp.tensor, dict[str, tuple[int, ...]]]:
    ...
    for name in ("embedding", "conv1", "pool1", "conv2", "pool2", "dense"):
...
def run_qcnn(
    ...
    result = {
        ...
        "pool1_params": np.asarray(structured_params["pool1"], dtype=float),
        "conv2_params": np.asarray(structured_params["conv2"], dtype=float),
        "pool2_params": np.asarray(structured_params["pool2"], dtype=float),
        "dense_params": np.asarray(structured_params["dense"], dtype=float),
        "active_wires_by_stage": [[0, 1, 2, 3], [1, 3], [3]],
        "y_train": y_train,
        "y_test": y_test,
