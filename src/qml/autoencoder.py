...
def run_quantum_autoencoder(
    ...
    The encoder is trained to compress information into the leading
    ``latent_qubits`` by maximizing the probability that the remaining trash
    qubits are measured in ``|0...0>``. Reconstruction is evaluated by
    postselecting the encoded state onto the trash-zero subspace, renormalizing
    that compressed state, and decoding with the adjoint of the learned encoder.
    This makes reconstruction fidelity sensitive to information lost during
    compression instead of cancelling the encoder by construction.
    """
    n_qubits = 4
    ...
    @qml.qnode(dev_eval)
    def encoded_state(x, current_params):
        _prepare_state(x, wires=wires, family=family)
        apply_hardware_efficient_ansatz(current_params, wires=wires)
        return qml.state()

    @qml.qnode(dev_eval)
    def decoded_state(compressed_state, current_params):
        qml.StatePrep(compressed_state, wires=wires)
    ...
    def _postselect_trash_zero(state: np.ndarray) -> tuple[np.ndarray, float]:
        state = np.asarray(state, dtype=complex).reshape([2] * n_qubits)
        mask = np.zeros_like(state, dtype=complex)
        index = [slice(None)] * n_qubits
        for wire in trash_wires:
            index[wire] = 0
        mask[tuple(index)] = state[tuple(index)]
        flat = mask.reshape(-1)
        probability = float(np.vdot(flat, flat).real)
        if probability <= 1e-12:
            return flat, probability
        return flat / np.sqrt(probability), probability

    def reconstruction_fidelity_single(x, current_params) -> float:
        compressed_state, success_probability = _postselect_trash_zero(
            encoded_state(x, current_params)
        )
        if success_probability <= 1e-12:
            return 0.0
        return _state_fidelity(original_state(x), decoded_state(compressed_state, current_params))
    ...
    train_reconstruction = np.asarray(
        [reconstruction_fidelity_single(x, params) for x in x_train],
        dtype=float,
    )
    test_reconstruction = np.asarray(
        [reconstruction_fidelity_single(x, params) for x in x_test],
        dtype=float,
    )
    ...
    result = {
        ...
        "reconstruction_method": "trash_zero_postselection_tied_decoder",
